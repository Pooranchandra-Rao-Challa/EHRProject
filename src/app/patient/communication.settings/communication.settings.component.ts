...
export class CommunicationSettingsComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    let patientId = this.user.PatientId;
    this.patientService.PatientProfile({PatientId:patientId}).subscribe((resp)=>{
      if(resp.IsSuccess){
  ...
  get HasEmail(): boolean {
    return this.patient.Email != null && this.patient.Email != 'No Email';
  }

  get HasCellNumber(): boolean {
    return this.patient.MobilePhone != null;
  }
  get HasHomeNumber(): boolean {
    return this.patient.PrimaryPhone != null;
  }

  UseHomeNumber() {
    let val = this.automatedVoiceForm.value;
    val.PhoneNumber =  this.patient.PrimaryPhone;
    val.ConfirmPhoneNumber =  this.patient.PrimaryPhone;
  ...
  UseCellNumber(isSms: boolean) {
    if (isSms) {
      ...
      val.PhoneNumber = this.patient.MobilePhone;
      val.ConfirmPhoneNumber =  this.patient.MobilePhone;
  ...
  UseEmail() {
    ...
    if (this.authService.viewModel.Patient.Email != 'No Email') {
      val.Email =  this.patient.Email;
      val.ConfirmEmail = this.patient.Email;
