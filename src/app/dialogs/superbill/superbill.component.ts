...
export class SuperbillDialogComponent implements OnInit {
  ...
  get ProcedureCost(): number {
    ...
    if (this.bill.EncounterInfo.CompletedProcedures) {
  ...
  copyBill(b: SuperBill) {
    ...
    if (this.bill?.Insurance?.DateOfBirth.split(' ')[0])
      this.bill.Insurance.dob = new Date(this.bill.Insurance.DateOfBirth.split(' ')[0]);
  ...
  setLocalVariables() {
    if (this.overlayref.data.encounterInfo != null) {
      ...
      this.bill.Procedures = this.bill.EncounterInfo.CompletedProcedures;
    }
    else if (this.overlayref.data.bill)
      this.bill = this.overlayref.data.bill;
    ...
    if (!this.bill.Coverage) this.bill.Coverage = {};
  }
